  ...
  const testKey = async (provider: keyof typeof apiKeys) => {
    ...
    try {
      if (provider === 'anthropic') {
        ...
        // For other providers, just check format for now
        setKeyStatuses({
          ...keyStatuses,
          [provider]: {
            status: 'valid',
            message: 'Format OK (full validation coming soon)',
          },
        });
      }
    } catch (error) {
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {activeTab === 'api-keys' && (
              <div className="space-y-6">
                ...
                {/* OpenAI - Kept for compatibility but note OpenRouter is better */}
                <div className="space-y-3 opacity-60">
                  <label className="block text-sm font-medium text-zinc-200">
                    OpenAI (Legacy - Use OpenRouter instead)
                  </label>
                  <div className="flex gap-2">
                    <input
                      type="password"
                      ...
                      placeholder="sk-..."
                      className="flex-1 px-3 py-2 bg-zinc-800 border border-zinc-600 rounded-lg text-zinc-50 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono text-sm"
                    />
                    <a
                ...
                <div className="space-y-3 opacity-60">
                  <label className="block text-sm font-medium text-zinc-200">
                    Google AI (Legacy - Use OpenRouter instead)
                  </label>
                  <div className="flex gap-2">
                    <input
                      type="password"
