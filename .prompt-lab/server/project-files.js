...
const WORKSPACE_ROOT = process.env.WORKSPACE_DIR || '/home/runner/workspace';

// Configuration - Read from .env or allow all
function getAllowedProjects() {
  const allowed = process.env.ALLOWED_PROJECTS;
  if (!allowed || allowed.trim() === '') {
    return null; // null means allow all non-empty directories
  }
  return allowed.split(',').map(p => p.trim()).filter(Boolean);
}

const ALLOWED_PROJECTS = getAllowedProjects();
...
export async function getProjects() {
  try {
    ...
    if (ALLOWED_PROJECTS) {
      projects = projects.filter(e => ALLOWED_PROJECTS.includes(e.name));
    }
    ...
    for (const entry of projects) {
      try {
        const projPath = path.join(WORKSPACE_ROOT, entry.name);
        const contents = await fs.readdir(projPath);
        if (contents.length > 0) {
          nonEmptyProjects.push(entry.name);
        }
      } catch (err) {
        // Skip on error
      }
...
export async function getProjectTree(projectName) {
  if (ALLOWED_PROJECTS && !ALLOWED_PROJECTS.includes(projectName)) {
    return null;
  }
